...
unsigned long omega_init(void)
{
	...
	if (error) goto exit;
	error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh7_en, reg_top_gpioh7_en_pos, reg_top_gpioh7_en_len, 1);
	if (error) goto exit;
	error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh7_on, reg_top_gpioh7_on_pos, reg_top_gpioh7_on_len, 1);
	if (error) goto exit;
...
unsigned long omega_setfreq(unsigned int bw_kHz, unsigned int rf_freq_kHz)
{
    ...
    if(rf_freq_kHz<c_fUHF_min)
		error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh7_o, reg_top_gpioh7_o_pos, reg_top_gpioh7_o_len, 0);
	else
		error = Standard_writeRegisterBits (Afatech_OMEGA_demodulator, Processor_LINK, p_reg_top_gpioh7_o, reg_top_gpioh7_o_pos, reg_top_gpioh7_o_len, 1);
	if (error) goto exit;
exit:
	return (error);
}
