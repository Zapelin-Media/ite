...
void SET_LE(Modulator*    modulator){
	if(modulator->systemConfig.loLe != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loLe+1, 1); 		
	}
}

void CLR_LE(Modulator*    modulator){
	if(modulator->systemConfig.loLe != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loLe+1, 0); 		
	}
}
void SET_SCL(Modulator*    modulator){
	if(modulator->systemConfig.loClk != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loClk+1, 1); 		
	}
}
void CLR_SCL(Modulator*    modulator){
	if(modulator->systemConfig.loClk != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loClk+1, 0); 		
	}
}

void SET_DATA(Modulator*    modulator){
	if(modulator->systemConfig.loData != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loData+1, 1); 		
	}
}
void CLR_DATA(Modulator*    modulator){
	if(modulator->systemConfig.loData != UNUSED){
		IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loData+1, 0); 		
	}
}
...
Dword ADF4351_busInit (Modulator*    modulator) {
	...
	if(modulator->systemConfig.loClk != UNUSED){ //output
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loClk+2, 1);//gpiox_en
		if (error) goto exit;
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loClk+3, 1);//gpiox_on
		if (error) goto exit;		
	}

	if(modulator->systemConfig.loData != UNUSED){ //output
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loData+2, 1);//gpiox_en
		if (error) goto exit;
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loData+3, 1);//gpiox_on
		if (error) goto exit;		
	}

	if(modulator->systemConfig.loLe != UNUSED){ //output
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loLe+2, 1);//gpiox_en
		if (error) goto exit;
		error = IT9510_writeRegister ((IT9510INFO*)modulator, Processor_LINK, modulator->systemConfig.loLe+3, 1);//gpiox_on
		if (error) goto exit;	
	}
exit:
	return error;
