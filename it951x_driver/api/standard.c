...
Dword Standard_computeCrystal (
	...
	if(demodulator !=NULL)
		*crystal = (Long) Standard_divider ((Dword) crystalFrequency, 1000000ul, 19ul);
...
)
{
	...
	if(demodulator !=NULL)
		*adc = (Long) Standard_divider ((Dword) adcFrequency, 1000000ul, 19ul);
...
Dword Standard_computeFcw (
	...
	controlWord = (Long) Standard_divider ((Dword) adcFreqSample, (Dword) adcFreq, 23ul);
...
Dword Standard_selectBandwidth (
	...
	Dword error = Error_NO_ERROR;
	Dword BWerror = Error_NO_ERROR;
	Dword coeff1_2048Nu = 0;
	Dword coeff1_4096Nu = 0;
	Dword coeff1_8191Nu = 0;
	Dword coeff1_8192Nu = 0;
	Dword coeff1_8193Nu = 0;
	Dword coeff2_2k = 0;
	Dword coeff2_4k = 0;
	Dword coeff2_8k = 0;
	Word bfsfcw_fftindex_ratio = 0;
	Word fftindex_bfsfcw_ratio = 0;
...
Dword Standard_setFrequency (
	...
	if (tunerDescription.tunerId >= 0x63 && tunerDescription.tunerId <= 0x65){
		error = Standard_writeRegister(demodulator, Processor_OFDM, 0x019b, band);
		if (error) goto exit;
	}
...
Dword Standard_getChannelModulation (
	...
	error = Standard_readRegister (demodulator, Processor_OFDM, bfsfcw_fftindex_ratio_7_0, &fftindex);
	if (error) goto exit;
	
	if(clk_strap == 1){
		if(fftindex == 0x80)
			channelModulation->bandwidth = Bandwidth_3M;
		else if(fftindex == 0xa0)
			channelModulation->bandwidth = Bandwidth_4M;
		else if(fftindex == 0x33)
			channelModulation->bandwidth = Bandwidth_2_5M;
		else if(fftindex == 0x40)
			channelModulation->bandwidth = Bandwidth_2M;
		else
			channelModulation->bandwidth = Bandwidth_UNKNOW;

	}else{
		error = Standard_readRegisterBits (demodulator, Processor_OFDM, g_reg_bw, reg_bw_pos, reg_bw_len, &temp);
		if (error) goto exit;

		if(temp == 3 && fftindex == 0x6e)
			channelModulation->bandwidth = Bandwidth_4M;
		else
...
Dword Standard_acquireChannel (
	...
	BWerror = Standard_selectBandwidth (demodulator, bandwidth, demod->adcFrequency);
	//if (error) goto exit;
...
Dword Standard_initialize (
	...
	if (demod->booted == True) {
		...
		/** Compute ADC and load them to device */
		error = Standard_computeCrystal ((Long) demod->crystalFrequency * 1000, &crystal);
		if (error) goto exit;
		...
		/** Compute ADC and load them to device */
		error = Standard_computeAdc ((Long) demod->adcFrequency, &adc);
		if (error) goto exit;
