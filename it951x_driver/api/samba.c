...
unsigned long samba_init(void)
{
	...
	//if(isCATV == 1){
		...
		error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh7_en, reg_top_gpioh7_en_pos, reg_top_gpioh7_en_len, 1);
		if (error) goto exit;
		error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh7_on, reg_top_gpioh7_on_pos, reg_top_gpioh7_on_len, 1);
		if (error) goto exit;
	//}
	//LNA always Lo
	error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh6_en, reg_top_gpioh6_en_pos, reg_top_gpioh6_en_len, 1);
	if (error) goto exit;
	error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh6_on, reg_top_gpioh6_on_pos, reg_top_gpioh6_on_len, 1);
	if (error) goto exit;
	error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh6_o, reg_top_gpioh6_o_pos, reg_top_gpioh6_o_len, 0);
exit:
	return (error);
}
unsigned long samba_setfreq(unsigned int bw_kHz, unsigned int rf_freq_kHz)
{
	...
	//if(isCATV == 1){
		...
		if(rf_freq_kHz<c_fUHF_min)
			error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh7_o, reg_top_gpioh7_o_pos, reg_top_gpioh7_o_len, 0);
		else
			error = Standard_writeRegisterBits (Afatech_SAMBA_demodulator, Processor_LINK, p_reg_top_gpioh7_o, reg_top_gpioh7_o_pos, reg_top_gpioh7_o_len, 1);
		if (error) goto exit;
	//}
