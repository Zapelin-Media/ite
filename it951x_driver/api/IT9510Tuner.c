...
int IT9510Tuner_setIO(const int THETA){
   int reverse;
   int sign;
...
Dword IT9510Tuner_interpolation(
	...
	)
{
  // Using binary search to find the frequency interval in the table
	...
	if(fIn <= (int)(modulator->calibrationInfo.ptrIQtableEx[0].frequency)){
		//idx = 0;
		(*ptrdAmp) = modulator->calibrationInfo.ptrIQtableEx[0].dAmp;
		(*ptrdPhi) = modulator->calibrationInfo.ptrIQtableEx[0].dPhi;
		goto exit;
	}else if(fIn >= (int)(modulator->calibrationInfo.ptrIQtableEx[TABLE_NROW - 1].frequency)){
		(*ptrdAmp) = modulator->calibrationInfo.ptrIQtableEx[TABLE_NROW - 1].dAmp;
		(*ptrdPhi) = modulator->calibrationInfo.ptrIQtableEx[TABLE_NROW - 1].dPhi;
		goto exit;
	}else{ 
...
Dword IT9510Tuner_setIQCalibration(
    ...
    error = IT9510Tuner_interpolation(modulator, frequency, ptrdAmp, ptrdPhi);
	...
	test2 = IT9510Tuner_setIO(dPhi); 

	c1 = (int) (alpha * alpha/BYTESHIFT);
	c2 = (int) (1*alpha*beta/16*test/4096/BYTESHIFT);
	c3 = (int) (alpha*beta/16*test2/4096/BYTESHIFT);
...
exit:
	error = IT9510_writeRegisters(modulator, Processor_OFDM, reg, 6, val);
