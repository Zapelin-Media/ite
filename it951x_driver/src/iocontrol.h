...
typedef struct {
    __u32				pDCtableAddr;			// Byte*
	__u32				pOFStableAddr;		// Byte*
	Word				tableGroups;			//Number of DCtable groups;
    __u32				error;
    Byte				reserved[16];
...
typedef struct {
    Word			slaveAddress;
    Word			startAddressOffset;
    Word			writeSize;
    Byte			buffer[256];
    __u32			error;
    Byte			reserved[16];
} IT9560WriteEepromValuesRequest, *PIT9560WriteEepromValuesRequest;
...
typedef struct {
    ...
    __u32			error;
    Byte			reserved[16];
} IT9560ReadEepromValuesRequest, *PIT9560ReadEepromValuesRequest;
...
typedef struct {
    ...
    __u32			error;
    Byte			reserved[16];
} IT9560WriteRegistersRequest, *PIT9560WriteRegistersRequest;
...
typedef struct {
    ...
    __u32			error;
    Byte			reserved[16];
} IT9560ReadRegistersRequest, *PIT9560ReadRegistersRequest;

typedef struct {
	Byte			FwVersion[8];
    __u32			error;
    Byte			reserved[16];
} IT9560GetStatisticRequest, *PIT9560GetStatisticRequest;
...
typedef struct {
    ...
    __u32				error;
    Byte				reserved[16];
} TxAcquireChannelDualRequest, *PTxAcquireChannelDualRequest;
...
/**
 * Acquire Channel Dual IT9507/IT9503 in modulator.
 * Paramters:   TxAcquireChannelDual struct
 */
#define IOCTL_ITE_MOD_ACQUIRECHANNELDUAL \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_STANDARD + 0x3C, TxAcquireChannelDualRequest)
...
/**
 * Enable Set IT9560 Get Statistic From API.
 * Paramters:	IT9560GetStatisticRequest struct
 */
#define IOCTL_ITE_BB_IT9560_GETSTATISTIC \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_OTHER + 0x17, IT9560GetStatisticRequest)

/**
 * Enable Set IT9560 Write EEPROM Data From API.
 * Paramters:	IT9560WriteEepromValuesRequest struct
 */
#define IOCTL_ITE_BB_IT9560_WRITEEEPROMVALUES \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_OTHER + 0x18, IT9560WriteEepromValuesRequest)

/**
 * Enable Set IT9560 Read EEPROM Data From API.
 * Paramters:	IT9560WriteEepromValuesRequest struct
 */
#define IOCTL_ITE_BB_IT9560_READEEPROMVALUES \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_OTHER + 0x19, IT9560ReadEepromValuesRequest)

/**
 * Enable Set IT9560 Write Register From API.
 * Paramters:	IT9560WriteRegistersRequest struct
 */
#define IOCTL_ITE_BB_IT9560_WRITEREGISTERS \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_OTHER + 0x20, IT9560WriteRegistersRequest)

/**
 * Enable Set IT9560 Read Register From API.
 * Paramters:	IT9560ReadRegistersRequest struct
 */
#define IOCTL_ITE_BB_IT9560_READREGISTERS \
	_IOW(AFA_IOC_MAGIC, IOCTRL_ITE_GROUP_OTHER + 0x21, IT9560ReadRegistersRequest)
