 ...
 *
 * Abstract:
 *   The core of IT951x serial driver.
 */
...
static DEFINE_MUTEX(it951x_urb_kill);
static DEFINE_MUTEX(it951x_rb_mutex);
static Byte tx_usb_handles[TX_MAX_USB_HANDLE_NUM] = {0};	
static Byte rx_usb_handles[RX_MAX_USB_HANDLE_NUM] = {0};	

#define to_afa_dev(d) container_of(d, struct it951x_dev, kref)

static struct usb_driver  it951x_driver;

struct usb_device_id it951x_usb_id_table[] = {
		{ USB_DEVICE(0x048D,0x9517) },
		{ 0},		/* Terminating entry */
};
MODULE_DEVICE_TABLE(usb, it951x_usb_id_table);

/* AirHD */
Dword Tx_RMRingBuffer(struct it951x_urb_context *context, Dword dwDataFrameSize)
{
	struct it951x_dev *dev = context->dev;
	//Dword dwBuffLen = 0;
...
Dword Tx_RMRingBuffer_low_brate(struct it951x_urb_context *context, Dword dwDataFrameSize)
{
	struct it951x_dev *dev = context->dev;
	//Dword dwBuffLen = 0;
	unsigned long flags = 0;	
...
/* AirHD for cmd */
#if RETURN_CHANNEL
Dword Tx_RMRingBuffer_cmd(struct it951x_urb_context *context, Dword dwDataFrameSize)
{
	struct it951x_dev *dev = context->dev;
	//deb_data("enter %s", __func__);
...
static void
rx_free_urbs(struct it951x_dev *dev)
{
	int i;
...
static void
tx_free_urbs(struct it951x_dev *dev)
{
	int i;
	//deb_data("Enter %s Function\n",__FUNCTION__);
...
/*
static void
rx_kill_busy_urbs(struct it951x_dev *dev)
{
	int i;
	//deb_data("Enter %s Function\n",__FUNCTION__);
...
static void
tx_kill_busy_urbs(struct it951x_dev *dev)
{
	int i;
	//deb_data("Enter %s Function\n",__FUNCTION__);
...
*/
static int rx_stop_urb_transfer(struct it951x_dev *dev)
{
#if 0
	int i;
#endif
	...
	//mutex_lock(&it951x_urb_kill);	     /* if urb doesn't call back, kill it. */
	for (i = 0; i < URB_COUNT_RX; i++) {
...
static int tx_stop_urb_transfer(struct it951x_dev *dev)
{
	deb_data("%s()\n", __func__);
...
#if RETURN_CHANNEL
static int tx_stop_urb_transfer_cmd(struct it951x_dev *dev)
{
	deb_data("%s()\n", __func__);
 ...
 * Description:
 * Successful submissions return 0(Error_NO_ERROR) and submited buffer length.
...
{
	struct it951x_urb_context *context = purb->context;
...
/* AirHD */
static int tx_start_urb_transfer(struct it951x_dev *dev)
{
...
/* AirHD_CMD */
#if RETURN_CHANNEL
static int tx_start_urb_transfer_cmd(struct it951x_dev *dev)
{
	int ret = -ENOMEM;
...
#endif
static void rx_start_urb_transfer(struct it951x_dev *dev)
{
...
static int it951x_usb_open(struct inode *inode, struct file *file)
{
	struct it951x_dev *dev;
	...
	deb_data("it951x_usb_rx_open function\n");
	mainsubminor = iminor(inode);
	subminor = iminor(inode);
	interface = usb_find_interface(&it951x_driver, subminor);

try:
	while (!interface) {
		subminor++;
		interface = usb_find_interface(&it951x_driver, subminor);
		if (subminor >= mainsubminor + USB_it91xx_MINOR_RANGE)
			break;
	}	
...
static int it951x_usb_tx_open(struct inode *inode, struct file *file)
{
	struct it951x_dev *dev;
	struct usb_interface *interface;
	...
	deb_data("it951x_usb_tx_open function\n");
...
try:
	while (!interface) {
		subminor++;
		interface = usb_find_interface(&it951x_driver, subminor);
		if (subminor >= mainsubminor + USB_it951x_MINOR_RANGE)
			break;
	}		
...
//	IT9510_setIQtable ((IT9510INFO*) &(dev->DC.modulator), dev->DC.modulator.calibrationInfo);	
		
exit:
	return retval;
}

static int it951x_usb_release(struct inode *inode, struct file *file)
{
	struct it951x_dev *dev;
	int error, i;
	//int order;

	dev = (struct it951x_dev *)file->private_data;
	if (dev == NULL) {
		deb_data("dev is NULL\n");
		return -ENODEV;
	}
	...
	if(atomic_read(&dev->g_AP_use_rx) == 1) {   // Prevent multi-open the same minor of APs.
		...
		/* if rx urb doesn't call back, kill it. */
		for (i = 0; i < URB_COUNT_RX; i++) {   
			if(dev->rx_urbstatus[i] == 1){
				usb_kill_urb(dev->rx_urbs[i]);
			}
...
static int it951x_usb_tx_release(struct inode *inode, struct file *file)
{
	struct it951x_dev *dev;
	int error;

	//deb_data("it951x_usb_release function\n");	
	dev = (struct it951x_dev *)file->private_data;
	if (dev == NULL) {
		deb_data("dev is NULL\n");
		return -ENODEV;
...
int SetLowBitRateTransfer(struct it951x_dev *dev, void *parg)
{
...
int Thread_buffer_reset(void *DEV)
{
	...
	while(!kthread_should_stop()){
		wait_event_interruptible(dev->reset_wait, (dev->buffer_shift == 1) || (dev->DC.modulator.dev_disconnect == true));
		
		if(dev->DC.modulator.dev_disconnect == 1)
			break;
...
int RF_relock_thread(void *DEV)
{
	IT9510INFO *modulator = (IT9510INFO *) DEV;
	Byte is_lock = 0;
	DWORD error = Error_NO_ERROR;
	deb_data("- Create %s Thread -\n", __func__);
	
	while(!kthread_should_stop()) {
		if(modulator->start_detect_relock_status == 1) {
			is_lock = DL_DetectLockSignal((void*) modulator);
			if(is_lock == 0) {
				printk("RF No lock!! Relock signal\n");
				IT9510User_RFFC2072WriteRegister((RFFC2072INFO*) &modulator->RFFC2072, reg_PLL_CTRL, 0x822C);			
			}
		}
		msleep(1000);
	}	
...
static int it951x_usb_ioctl(struct inode *inode, struct file *file,
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
long it951x_usb_unlocked_ioctl(
	struct file *file,
	unsigned int cmd,
	unsigned long parg)
{
...
#if LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,35)
static int it951x_usb_tx_ioctl(struct inode *inode, struct file *file,
	unsigned int cmd,  unsigned long parg)
{
	struct it951x_dev *dev;
	...
	PTxSetDCTableRequest		pSetDCTableRequest;

	Byte *pIQtableUser, *pIQtableKernel;
	DCtable *pDCtableUser;
	DCtable *pOFStableUser;
...
/*
		...
		case IOCTL_ITE_MOD_SETDEVICETYPE:
			...
			pTxSetDeviceTypeRequest.error = IT9510User_getDeviceTypeSetting((IT9510INFO*) &(handle->modulator), pTxSetDeviceTypeRequest.DeviceType, &syscfg);
			if(pTxSetDeviceTypeRequest.error != Error_NO_ERROR)
				...
				deb_data("IT9510User_getDeviceTypeSetting ok\n\t- DeviceType id: %d\n", pTxSetDeviceTypeRequest.DeviceType);				
			...
			pTxSetDeviceTypeRequest.error = IT9510User_setSystemConfig((IT9510INFO*) &(handle->modulator), syscfg);
			if(pTxSetDeviceTypeRequest.error != Error_NO_ERROR)
				deb_data("IT9510User_setSystemConfig fail\n");	
			else
				deb_data("IT9510User_setSystemConfig ok\n");							

			/* re-init GPIO setting for new device type */
			pTxSetDeviceTypeRequest.error = IT9510User_Initialization((IT9510INFO*) &(handle->modulator));
			if(pTxSetDeviceTypeRequest.error != Error_NO_ERROR)		
				deb_data("IT9510User_Initialization fail\n");	
			else 														
				deb_data("IT9510User_Initialization ok\n");
			...
			if(pIQtableUser != NULL) {
				...
				if(dev->DC.calibrationInfo.tableGroups != tableGroups) {		// Reallocate table.
					...
					if(dev->DC.calibrationInfo.ptrIQtableEx) {
						free_pages((unsigned long) dev->DC.calibrationInfo.ptrIQtableEx, get_order(dev->DC.calibrationInfo.tableGroups) * sizeof(IQtable));
						dev->DC.calibrationInfo.ptrIQtableEx = NULL;
					}
					dev->DC.calibrationInfo.tableGroups = tableGroups;
					dev->DC.calibrationInfo.ptrIQtableEx = (IQtable*)__get_free_pages(GFP_KERNEL, get_order(tableGroups) * sizeof(IQtable));
				}
...
#endif
				pSetIQTableRequest->error = IT9510_setIQtable ((IT9510INFO*) &(dev->DC.modulator), dev->DC.calibrationInfo);
		...
		case IOCTL_ITE_MOD_SETDCTABLE:
			pSetDCTableRequest = (PTxSetDCTableRequest) parg;
			pDCtableUser = (DCtable*) pSetDCTableRequest->pDCtableAddr;
			pOFStableUser = (DCtable*) pSetDCTableRequest->pOFStableAddr;
			tableGroups = pSetDCTableRequest->tableGroups;
#if 0
			deb_data("\n[SETDCTable] DCTable:\n");
			for(i = 0; i < tableGroups; i++) {
				 deb_data("[SETDCTable] DCTable[%d]: startFrequency = %lu\n",i, pDCtableUser[i].startFrequency);
				 deb_data("[SETDCTable] DCTable[%d]: i = %d\n",i, pDCtableUser[i].i);
				 deb_data("[SETDCTable] DCTable[%d]: q = %d\n",i, pDCtableUser[i].q);
			}
			deb_data("\n[SETDCTable] OFSTable:\n");
			for(i = 0; i < tableGroups; i++) {
				 deb_data("[SETDCTable] OFSTable[%d]: startFrequency = %lu\n",i,pOFStableUser[i].startFrequency);
				 deb_data("[SETDCTable] OFSTable[%d]: i = %d\n",i, pOFStableUser[i].i);
				 deb_data("[SETDCTable] OFSTable[%d]: q = %d\n",i, pOFStableUser[i].q);
			}
#endif
			/* If table group is difference, reallocate table */
			if(dev->DC.dcInfo.tableGroups != tableGroups) {
				deb_data("[SETDCTable] Reallocate DC & OFS Table.\n");
				if(dev->DC.dcInfo.ptrDCtable) {	//	realloc DC table
					free_pages((unsigned long) dev->DC.dcInfo.ptrDCtable, get_order(dev->DC.dcInfo.tableGroups)*sizeof(DCtable));
					dev->DC.dcInfo.ptrDCtable = NULL;
					dev->DC.dcInfo.ptrDCtable = (DCtable*)__get_free_pages(GFP_KERNEL, get_order(tableGroups)*sizeof(DCtable));
				}
				if(dev->DC.dcInfo.ptrOFStable) {	//	realloc OFS table
					free_pages((unsigned long) dev->DC.dcInfo.ptrOFStable, get_order(dev->DC.dcInfo.tableGroups)*sizeof(DCtable));
					dev->DC.dcInfo.ptrOFStable = NULL;
					dev->DC.dcInfo.ptrOFStable = (DCtable*)__get_free_pages(GFP_KERNEL, get_order(tableGroups)*sizeof(DCtable));
				}
			}
			
			dev->DC.dcInfo.tableGroups = tableGroups;
			copy_from_user(dev->DC.dcInfo.ptrDCtable, pDCtableUser, tableGroups * sizeof(DCtable));
			copy_from_user(dev->DC.dcInfo.ptrOFStable, pOFStableUser, tableGroups * sizeof(DCtable));

			pSetDCTableRequest->error = IT9510_setDCtable ((IT9510INFO*) &(dev->DC.modulator), dev->DC.dcInfo);
//			printk("pSetDCTableRequest->error: [%lu]\n", pSetDCTableRequest->error);
			return 0;
	}
...
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
long it951x_usb_tx_unlocked_ioctl(
	struct file *file,
	unsigned int cmd,
	unsigned long parg)
{
...
static ssize_t it951x_read(
	struct file *file, 
	char __user *buf,
	size_t count, 
	loff_t *ppos)
{
	struct it951x_dev *dev = (struct it951x_dev *)file->private_data;
	Dword Len = count;

	if (dev == NULL) 
		return -ENODEV;

	if(!dev->DC.modulator.dev_disconnect) 
	{
...
static ssize_t it951x_tx_write(
	struct file *file,
...
/*	if(dwError != 0) return dwError;
	...
	if(dev->is_use_low_brate) {
		//deb_data("it951x_tx_write: g_is_use_low_brate is open!\n");
		dwError = Tx_RingBuffer_low_brate(dev, user_buffer, &Len);
	}else{
		//deb_data("it951x_tx_write: g_is_use_low_brate is close!\n");
...
static struct file_operations it951x_usb_rx_fops ={
	.owner =		THIS_MODULE,
	.open =		it951x_usb_open,
	.read =		it951x_read,
	//.write =	it951x_write,
	.release =	it951x_usb_release,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
		.unlocked_ioctl = it951x_usb_unlocked_ioctl,
#else
		.ioctl = it951x_usb_ioctl,
#endif

};

static struct file_operations it951x_usb_tx_fops ={
	.owner =		THIS_MODULE,
	.open =		it951x_usb_tx_open,
	//.read =		it951x_read,
	.write =	it951x_tx_write, /*AirHD*/ 
	.release =	it951x_usb_tx_release,
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
		.unlocked_ioctl = it951x_usb_tx_unlocked_ioctl,
#else
		.ioctl = it951x_usb_tx_ioctl,
#endif
 ...
 */
#if IT9133Rx
static struct usb_class_driver it951x_class = {
//	.name =			"usb-it913x%d",
	.fops =			&it951x_usb_rx_fops,
	.minor_base = 	USB_it91xx_MINOR_BASE
};
#else
static struct usb_class_driver it951x_class = {
//	.name =			"usb-it917x%d",
...
static struct usb_class_driver it951x_class_tx = {
//	.name =			"usb-it951x%d",
	.fops =			&it951x_usb_tx_fops,
	.minor_base = 	USB_it951x_MINOR_BASE
};
...
static int it951x_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	struct it951x_dev *dev = NULL;
	struct urb *purb;	
	int retval = -ENOMEM;
	int i, order, error;
	...
	dev = kzalloc(sizeof(struct it951x_dev), GFP_KERNEL);
	dev->g_AP_use = 0;
	...
	dev->DC.calibrationInfo.ptrIQtableEx = (IQtable*)__get_free_pages(GFP_KERNEL, get_order(IQ_TABLE_NROW) * sizeof(IQtable));
	
	/* Malloc DC TB */
	dev->DC.dcInfo.tableGroups = 7;
	dev->DC.dcInfo.ptrDCtable = (DCtable*) __get_free_pages(GFP_KERNEL, get_order(dev->DC.dcInfo.tableGroups)*sizeof(DCtable));
	dev->DC.dcInfo.ptrOFStable = (DCtable*) __get_free_pages(GFP_KERNEL, get_order(dev->DC.dcInfo.tableGroups)*sizeof(DCtable));

	/* we can register the device now, as it is ready */
	usb_set_intfdata(intf, dev);
	deb_data("*** it951x usb device pluged in ***\n");
	...
	if(dev->DC.Rx_init_success)	{
		...
		if(it951x_class.name == NULL) 
			it951x_class.name = kzalloc(sizeof(char)*MAX_USB_HANDLE_NAME_SIZE, GFP_KERNEL);
		dev->rx_handle_num = usb_handle_set(rx_usb_handles, RX_MAX_USB_HANDLE_NUM);
		if(dev->rx_handle_num >= 0)
#if IT9133Rx	
			snprintf(it951x_class.name, MAX_USB_HANDLE_NAME_SIZE, "usb-it951x-rx%d", dev->rx_handle_num);	//usb-it913x
#else 
			snprintf(it951x_class.name, MAX_USB_HANDLE_NAME_SIZE, "usb-it951x-rx%d", dev->rx_handle_num);	// usb-it917x
#endif		
		else
			return -EBADF;

		/*  Register the device to usb core */
		retval = usb_register_dev(intf, &it951x_class);
		if (retval) {
	...
	if(dev->DC.Tx_init_success)	{
		/* Register Tx handle name */
		if(it951x_class_tx.name == NULL) 
			it951x_class_tx.name = kzalloc(sizeof(char)*MAX_USB_HANDLE_NAME_SIZE, GFP_KERNEL);
		dev->tx_handle_num = usb_handle_set(tx_usb_handles, TX_MAX_USB_HANDLE_NUM);
		if(dev->tx_handle_num >= 0)
			snprintf(it951x_class_tx.name, MAX_USB_HANDLE_NAME_SIZE, "usb-it951x%d", dev->tx_handle_num);
		else
			return -EBADF;

		/* Register Tx */
		intf->minor = -1;
		retval = usb_register_dev(intf, &it951x_class_tx);
		if (retval) {
			deb_data("Not able to get a minor for this device.");
			usb_set_intfdata(intf, NULL);
			//goto error;
...
#if RF_RELOCK_MONITOR	
	// for RFFC2072 board
	if(dev->DC.deviceType == 0x9C) {
		/* Init reset buffer thread */
		dev->DC.modulator.start_detect_relock_status = 0;	
		dev->DC.modulator.relock_thread = kthread_create(&RF_relock_thread, (void *)&dev->DC.modulator, "RF_Relock_Thread");
		if(!dev->DC.modulator.relock_thread || IS_ERR(dev->DC.modulator.relock_thread)){
			deb_data("\t Error: RF_Relock_Thread fail\n");
			retval = PTR_ERR(dev->DC.modulator.relock_thread);
			dev->DC.modulator.relock_thread = NULL;
			goto exit;
		}
		wake_up_process(dev->DC.modulator.relock_thread);	
	}
#endif
	/* device connect success */
	dev->DC.modulator.dev_disconnect = false;
	
	/* fixed issue for enhance K DC calibration value in orion board */
	temp.constellation = Constellation_64QAM;		// 64QAM
	temp.highCodeRate = CodeRate_7_OVER_8;			// 7/8
	temp.interval = Interval_1_OVER_32;				// 1/32
	temp.transmissionMode = TransmissionMode_8K;	// 8K
	error = IT9510_setTXChannelModulation((IT9510INFO*) &(dev->DC.modulator), &temp);
	if(error == 0 )
		printk("Default IT9510_setTXChannelModulation: success\n");
	else
		printk("Default IT9510_setTXChannelModulation: fail\n");	
...
static int it951x_suspend(struct usb_interface *intf, pm_message_t state)
{
	int error;
	struct it951x_dev *dev;
...
static int it951x_resume(struct usb_interface *intf)
{
	int retval = -ENOMEM;
	int error;
	struct it951x_dev *dev;
...
static void it951x_disconnect(struct usb_interface *intf)
{
	struct it951x_dev *dev;
...
//	mutex_lock(&it951x_mutex);
	...
	/* disable this flag to prevent Usb read/write Bus failure */
	dev->DC.modulator.dev_disconnect = true;
...
//	dev->disconnect = 1;
	...
	if(dev->DC.Rx_init_success)	{	
		...
		usb_deregister_dev(intf, &it951x_class);
	...
	if(dev->DC.Tx_init_success)	{		
		...
		usb_deregister_dev(intf, &it951x_class_tx);
...
//	mutex_unlock(&it951x_mutex);
 ...
 #if RF_RELOCK_MONITOR
 	if(dev->DC.deviceType == 0x9C) {	// for RFFC2072 board
		if(dev->DC.modulator.relock_thread != NULL) {
			kthread_stop(dev->DC.modulator.relock_thread);
			dev->DC.modulator.relock_thread = NULL;
		}
	}
#endif
...
static struct usb_driver it951x_driver = {
	...
	.name       = "usb-it951x",
	.probe      = it951x_probe,
	.disconnect = it951x_disconnect,//dvb_usb_device_exit,
	.id_table   = it951x_usb_id_table,
	.suspend    = it951x_suspend,
	.resume     = it951x_resume,
};

static int __init it951x_module_init(void)
{
	int result;

	//info("%s",__FUNCTION__);
	//deb_data("usb_it951x Module is loaded \n");

	if ((result = usb_register(&it951x_driver))) {
...
static void __exit it951x_module_exit(void)
{
	deb_data("usb_it951x Module is unloaded!\n");
	usb_deregister(&it951x_driver);
}

module_init (it951x_module_init);
module_exit (it951x_module_exit);

MODULE_AUTHOR("Jason Dong <jason.dong@ite.com.tw>");
MODULE_DESCRIPTION("DTV Native Driver for Device Based on ITEtech it951x");
