...
unsigned int IT9507_getLoFreqEx(Modulator*    modulator, unsigned int rf_freq_kHz)
{
	...
	if (rf_freq_kHz > 406000 && rf_freq_kHz <= 475000)  {
		IT9507_writeRegister (modulator, Processor_OFDM, 0xfb2c, 0x50);
				
		tmp_tg = (unsigned long)rf_freq_kHz * 8 * (unsigned long)g_fdiv;
	...
	} else if (rf_freq_kHz > 475000 && rf_freq_kHz <= 546000) {
		IT9507_writeRegister (modulator, Processor_OFDM, 0xfb2c, 0x52);
		    
		tmp_tg = (unsigned long)rf_freq_kHz * 30;
		tmp_m = (tmp_tg / (unsigned long)g_fxtal_kHz);
		tmp_cal = tmp_m * (unsigned long)g_fxtal_kHz;
		if ((tmp_tg-tmp_cal) >= (g_fxtal_kHz>>1)) {tmp_m = tmp_m+1;}
		mv = (unsigned int) (tmp_m);
	...
	} else if(rf_freq_kHz > 546000 && rf_freq_kHz <= 610000){

		IT9507_writeRegister (modulator, Processor_OFDM, 0xfb2c, 0x50);
		
		tmp_tg = (unsigned long)rf_freq_kHz * 18;
		tmp_m = (tmp_tg / (unsigned long)g_fxtal_kHz);
		tmp_cal = tmp_m * (unsigned long)g_fxtal_kHz;
		if ((tmp_tg-tmp_cal) >= (g_fxtal_kHz>>1)) {tmp_m = tmp_m+1;}
		mv = (unsigned int) (tmp_m);
...
Dword IT9507_setFrequency (
	...
	/*----- set_lo_freq -----*/
	f_temp = frequency - ((frequency/1000)*1000);
	if(frequency>406000 && frequency<610000 && f_temp==500){
		tmp = IT9507_getLoFreqEx(modulator, frequency);
	}else{
		error = IT9507_writeRegister (modulator, Processor_OFDM, 0xfb2c, 0x50);
		if (error) goto exit;
	...
	if(modulator->dcInfo.tableGroups != 0){
		...
		if(frequency < modulator->dcInfo.ptrDCtable[0].startFrequency){
			point = 0;
			...
			for(i=0;i<modulator->dcInfo.tableGroups-1;i++){
				
				upper = modulator->dcInfo.ptrDCtable[i].startFrequency;
				lower = modulator->dcInfo.ptrDCtable[i+1].startFrequency;
				if((frequency>=upper) && (frequency<lower)){
					
					if( (frequency-upper) > (lower - frequency) )
						point = i+1;
					else
						point = i;
					break;
		...
		error = IT9507_setDCCalibrationValue(modulator, modulator->dcInfo.ptrDCtable[point].i, modulator->dcInfo.ptrDCtable[point].q);
		if (error) goto exit;
...
Dword IT9507_setTPS (
	...
	error = IT9507_writeRegister (modulator, Processor_OFDM, p_eagle_reg_tps_cell_id_15_8, (Byte)(tps.cellid>>8));
	if (error) goto exit;

	error = IT9507_writeRegister (modulator, Processor_OFDM, p_eagle_reg_tps_cell_id_7_0, (Byte)(tps.cellid));
...
Dword IT9507_getTPS (
	...
	error = IT9507_readRegister (modulator, Processor_OFDM, p_eagle_reg_tps_cell_id_15_8, &temp);//get cell id
	if (error) goto exit;
	cellID = temp<<8;

	error = IT9507_readRegister (modulator, Processor_OFDM, p_eagle_reg_tps_cell_id_7_0, &temp);//get cell id
	cellID = cellID | temp;	
	pTps->cellid = cellID;
...
Dword IT9507_getChipType (
	...
	error = IT9507_readRegisters(modulator, Processor_LINK, chip_version_7_0+1, 2, var);
	if (error)	goto exit;

	error = IT9507_readRegister(modulator, Processor_LINK, 0xD805, &type);
	if (error)	goto exit;
...
Dword IT9507_setSpectralInversion(
	IN  Modulator*    modulator,
	IN  Bool		  isInversion
){
	Dword error = ModulatorError_NO_ERROR;
	...
	error = IT9507_writeRegister(modulator, Processor_OFDM, 0xf762, temp);
