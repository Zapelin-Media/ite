...
DeviceDescription HwDesc[] =
{
	...
	{NA,		RfEnable, NA,        NA,      NA,			  NA,           IrDa,   UvFilter	},	//12:4 ch Tx card
...
Dword EagleUser_acquireChannel (
	...
	// DC calibration
	error = IT9507_readRegister (modulator, Processor_LINK, 0x4979, &eeprom);//has eeprom ??
	if (error) goto exit;
	error = IT9507_readRegister (modulator, Processor_LINK, 0x49D6, &DCcalibration);//has DCcalibration ??
	if (error) goto exit;

	if((eeprom ==1) && ((DCcalibration & 0x80) == 0x80)){
		error = IT9507_readRegisters (modulator, Processor_LINK, 0x49D6, 12, DCvalue);//DC calibration value
		if (error) goto exit;
	
		error = IT9507_readRegisters (modulator, Processor_LINK, 0x49CC, 5, OFS_I_value);//OFS_I calibration value
		if (error) goto exit;

		error = IT9507_readRegisters (modulator, Processor_LINK, 0x49E2, 5, OFS_Q_value);//OFS_Q calibration value
		if (error) goto exit;


		if(frequency>=50000 && frequency<290000)
			index =1;
		else if(frequency>=290000 && frequency<300000)
			index =2;
		else if(frequency>=300000 && frequency<600000)
			index =3;
		else if(frequency>=600000 && frequency<650000)
			index =4;
		else
			index =5;	

		if((DCvalue[0] >> (index-1) ) &0x01)
			DC_I = DCvalue[(index*2)];
		else
			DC_I = DCvalue[(index*2)]*-1;

		if((DCvalue[1] >> (index-1) ) &0x01)
			DC_Q = DCvalue[1 + (index*2)];
		else
			DC_Q = DCvalue[1 + (index*2)]*-1;

		error = IT9507_setDCCalibrationValue(modulator, DC_I, DC_Q);
		if (error) goto exit;
