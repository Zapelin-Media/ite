...
Dword g_ITEAPI_TxDeviceExit(
    IN  Byte DevNo)
{
    return (close(g_hDriver[DevNo][0]));
}
	...
	IN  Byte			DevNo)
{
	...
	if (g_hDriver[DevNo][0] > 0) {
		request.pDCtableAddr = (__u32) pDCtable;
		request.pOFStableAddr = (__u32) pOFStable;
		request.tableGroups = tableGroups;
...
Dword g_ITEAPI_TxEnableTPSEncryption(
    IN  Dword key,
    IN  Byte  DevNo)
{
    Dword dwError = ERR_NO_ERROR;
    ...
    if (g_hDriver[DevNo][0] > 0) {
		request.key = (__u32) key;
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_MOD_ENABLETPSENCRYPTION, (void *)&request);
        dwError = (Dword) request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_TxDisableTPSEncryption(
    IN  Byte  DevNo)
{
    Dword dwError = ERR_NO_ERROR;
    ...
    if (g_hDriver[DevNo][0] > 0) {
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_MOD_DISABLETPSENCRYPTION, (void *)&request);
        dwError = (Dword) request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
/*
Dword g_ITEAPI_TxSetIT9560Data(
    IN  Byte*		pIT9560Bin,
    IN  Word		dataSize,
	IN  Byte		DevNo)
{
	Dword dwError = ERR_NO_ERROR;
    int result;
    TxSetIT9560FWRequest request; 

	if (g_hDriver[DevNo][0] > 0) {
		request.pFwDataAddr = (__u32) pIT9560Bin;
		request.dataSize = dataSize;
		result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_MOD_SETIT9560FW, (void *)&request);
        dwError = (Dword) request.error;
	}
	else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_IT9560GetStatistic(
	OUT	Byte		*FwVersion,
    IN	Byte		DevNo)
{
    Dword dwError = ERR_NO_ERROR;
    ...
    if (g_hDriver[DevNo][0] > 0) {
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_BB_IT9560_GETSTATISTIC, (void *)&request);
        memcpy (FwVersion, request.FwVersion, sizeof(FwVersion));
        dwError = request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_IT9560WriteEEPROM(
    IN	Word	slaveAddress,
    IN	Word	startAddressOffset,
    IN	Byte	*buffer,
    IN	Word	writeSize,
    IN	Byte	DevNo)
{
    ...
    if (g_hDriver[DevNo][0] > 0) {
        ...
        request.startAddressOffset = startAddressOffset;
        request.slaveAddress = slaveAddress;
        request.writeSize = writeSize;
        memcpy (request.buffer, buffer, writeSize);
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_BB_IT9560_WRITEEEPROMVALUES, (void *)&request);
        dwError = (Dword) request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_IT9560ReadEEPROM(
    IN	Word	slaveAddress,
    IN	Word	startAddressOffset,
    OUT	Byte	*buffer,
    IN	Word	readSize,
    IN	Byte	DevNo)
{
    ...
    if (g_hDriver[DevNo][0] > 0) {
        ...
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_BB_IT9560_READEEPROMVALUES, (void *)&request);
        memcpy (buffer, request.buffer, readSize);
        dwError = (Dword) request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_IT9560WriteRegisters(
    IN	Dword		dwRegAddr,
    ...
    IN	Byte		DevNo)
{
    ...
    if (g_hDriver[DevNo][0] > 0) {
        ...
        memcpy (request.buffer, buffer, bufferLength);
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_BB_IT9560_WRITEREGISTERS, (void *)&request);
        dwError = (Dword) request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
...
Dword g_ITEAPI_IT9560ReadRegisters(
    IN	Dword		dwRegAddr,
    IN	Processor	processor,
    IN	Byte		bufferLength,
    OUT	Byte		*buffer,
    IN	Byte		DevNo)
{
    ...
    if (g_hDriver[DevNo][0] > 0) {
        ...
        request.registerAddress	= dwRegAddr;
        request.bufferLength		= bufferLength;
        result = ioctl(g_hDriver[DevNo][0], IOCTL_ITE_BB_IT9560_READREGISTERS, (void *)&request);
        memcpy (buffer, request.buffer, request.bufferLength);
        dwError = request.error;
    }
    else {
        dwError = ERR_NOT_IMPLEMENTED;
